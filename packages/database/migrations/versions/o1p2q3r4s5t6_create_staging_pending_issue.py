...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('tech_stack_weight', sa.Float(), nullable=False, server_default='0.0'),
        sa.Column('q_score', sa.Float(), nullable=False, server_default='0.0'),
        
        sa.Column('state', sqlmodel.sql.sqltypes.AutoString(), nullable=False, server_default='open'),
        ...
        sa.CheckConstraint("status IN ('pending', 'processing', 'completed', 'failed')", name='ck_pending_issue_status'),
        schema='staging'
    )
...
def downgrade() -> None:
    ...
    op.drop_index('ix_pending_issue_created_at', table_name='pending_issue', schema='staging')
    op.drop_index('ix_pending_issue_status_pending', table_name='pending_issue', schema='staging')
